...
// story mode
func  GetLevelStory ( level: Int ) -> ( [String: Double], [String: Double], [String], [String:[String: Int]], Int, [String], [String] ) {

    var state_list: [[String: Double]] = []
    var success_condition_list: [[String: Double]] = []
    var qubits_used_list: [[String]] = []
    var allowed_gates_list: [[String:[String: Int]]] = []
    var intro: [[String]] = []
    var outro: [[String]] = []
    ...
    allowed_gates_list.append( [ "a":["x":0,"z":0,"h":0], "b":["x":0,"z":0,"h":0], "both":["cz":0] ] )
    ...
    //23
    state_list.append( MakeInitial(target_state:["XI":0, "ZI":1,"XX":0.0,"XZ":0.0,"ZX":0.0,"ZZ":1.0,"YY":0.0,"IX":0.0, "IZ":1.0],
                       inverse_solution:[["qdg","0"],["qdg","0"],["qdg","0"]]) )
    success_condition_list.append( ["ZI":1.0] )
    qubits_used_list.append( [ "0", "1" ] )
    ...
    //24
    state_list.append( MakeInitial(target_state:["XI":0, "ZI":1,"XX":0.0,"XZ":0.0,"ZX":0.0,"ZZ":1.0,"YY":0.0,"IX":0.0, "IZ":1.0],
                                   inverse_solution:[["qdg","1"],["qdg","1"],["qdg","1"]]) )
    success_condition_list.append( ["IZ":1.0] )
    qubits_used_list.append( [ "0", "1" ] )
    ...
    //25
    state_list.append( MakeInitial(target_state:["XI":0, "ZI":1,"XX":0.0,"XZ":0.0,"ZX":0.0,"ZZ":1.0,"YY":0.0,"IX":0.0, "IZ":1.0],
                                   inverse_solution:[["qdg","0"],["qdg","0"],["qdg","1"]]) )
    success_condition_list.append( ["IZ":1.0] )
    qubits_used_list.append( [ "0", "1" ] )
    ...
    let level_num = state_list.count

    return ( state_list[level], success_condition_list[level], qubits_used_list[level], allowed_gates_list[level], level_num, intro[level], outro[level] )

}

// sandbox
func  GetLevelSandbox ( level: Int ) -> ( [String: Double], [String: Double], [String], [String:[String: Int]], Int, [String], [String] ) {
    ...
    qubits_used_list.append( [ "0", "1" ] )
    allowed_gates_list.append( [ "0":["x":0,"z":0,"h":0,"bloch":0,"q":0,"qdg":0,"cx":0], "1":["x":0,"z":0,"h":0,"bloch":0,"q":1,"qdg":1,"cx":0], "both":["unbloch":0,"cz":0] ] )
...
// image superposer
func  GetLevelSuperposer ( level: Int, string1: String, string2: String ) -> ( [String: Double], [String: Double], [String], [String:[String: Int]], Int, [String], [String] ) {
    ...
    var definite: [String:[String]] = ["0":[],"1":[]]
    var superpos: [String:[String]] = ["same":[],"diff":[]]
    var first_super = -1
    for j in 0...3 {

        let index = string1.index(string1.startIndex, offsetBy: j)

        if string1[index]==string2[index] {
            definite[String(string1[index])]!.append( String(j) )
        } else {
            if first_super == -1 {
                first_super = j
                superpos["same"]!.append( String(j) )
            } else {
                let first_index = string1.index(string1.startIndex, offsetBy: first_super)
                if string1[index]==string1[first_index] {
                    superpos["same"]!.append( String(j) )
                } else {
                    superpos["diff"]!.append( String(j) )
                }
            }
        }
    ...
    if first_super != -1 {
		...
		//                "XX":0.0,"XZ":0.0,"ZX":-1.0,"ZZ":0.0,"YY":0.0,
		//                "IX":0.0, "IZ":0.0] )
	    //success_condition_list.append( ["ZI":0.0,] )
	    //qubits_used_list.append( [ "b[4]", "b[0]" ] )
	    //allowed_gates_list.append( [ "b[4]":["x":0,"z":0,"h":0], "b["+String(first_super)+"]":["z":0], "both":["cz":0] ] )
	    //intro.append( ["Now b[4] has done its job, we return it to its initial state",
            //               "That means making the top blue box empty",
            //               "We'll need one of the qubits in the superposition to help us disentangle it"] )
            //outro.append( ["Great!",
            //               "Copy the QISKit program into the notebook. Do this with a right click, rather than ctrl-C"] )
