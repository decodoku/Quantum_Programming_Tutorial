...
def GetLevelRandom () :
  ...
  # randomly generate 10 leevls
  for _ in range(10):

    # randomly choose an initial state (out of two possibilities)
    if random.choice([True, False]):
      state = {"XI":1.0, "ZI":1.0,
                        "XX":0.0,"XZ":1.0,"ZX":0.0,"ZZ":1.0,"YY":0.0,
                        "IX":0.0, "IZ":1.0}
    else:
      state = {"XI":0.0, "ZI":1.0,
                        "XX":0.0,"XZ":0.0,"ZX":1.0,"ZZ":1.0,"YY":0.0,
                        "IX":1.0, "IZ":1.0}
    # copy for target state
    target = copy.deepcopy(state)

    # shuffle the states
    for shuffled_state in [state,target]:
        for _ in range(10):
          for qubit_pos in ["0","1"]:
            shuffled_state = ApplyGate( shuffled_state, random.choice(["x","z","h"]), qubit_pos )
            shuffled_state = ApplyGate( shuffled_state, "cz", qubit_pos )

    state_list.append( state )
    success_condition_list.append( target )
    qubits_used_list.append( [ "0", "1" ] )
    
    allowed_gates_list.append( { "0":{"x":0,"h":0}, "1":{"x":0,"h":0}, "both":{"cz":0} } )
  
    lines = MakeGrid ( target, 2, 2, False )
    target_string = "\n"
    for line in lines:
        target_string += "  " + line + "\n"
    
    intro.append( ["The target state is", target_string] )
